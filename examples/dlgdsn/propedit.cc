...
bool IntegerEditor(uipointer& value, TPoint place, TGroup * host)
{
   bool rst = false;
   char strval[20];
...
#define _constsel_(list, editor)                                             \
          rec.items = list();                                                \
          rec.selection = list()->getIndex((ushort)(uipointer)ldata);              \
          chg = (execDialog(editor(), &rec) == cmOK);                        \
   ...
   switch(map->editorType)
   {
       ...
       case etIntegerEditor:
          ...
          if (vtCurrent == vtListBox && map->index == 14)
               i = (short)(uipointer)ldata; else i = (uipointer)ldata;  // JACS, 64 bit support
