...
dynamic-lib: intl-dummy
	...
	-cd makes; rm -f librhtv.so; ln -s librhtv.so.2.2.2 librhtv.so
	-cd makes; rm -f librhtv.so.2; ln -s librhtv.so.2.2.2 librhtv.so.2
...
install-dynamic: dynamic-lib
	...
	rm -f $(libdir)/librhtv.so.2.2.2
	cd $(libdir); ln -s librhtv.so.2.2.2 librhtv.so
	$(INSTALL) -m 0644 makes/librhtv.so.2.2.2 $(libdir)
	strip --strip-debug $(libdir)/librhtv.so.2.2.2
	-ldconfig
